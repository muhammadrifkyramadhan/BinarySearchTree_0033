...
public:
	...
	void insert(string element) //Inset a node in the binary search tree
	{
		...
		search(element, parent, currentNode); //Locate the node wich be the parent of the node to be inserted
		...
		{
			parent->leftchild = newNode; // Mark the left child of the parent point to the new node
		}
	...
	{
		//This function searches the current of the specified Node as well as the current Node its parrent
		currentNode = ROOT;
		parent = NULL;
